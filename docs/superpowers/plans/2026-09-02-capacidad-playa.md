...
- [x] Marcar una OT como "Retirado" la saca de la cuenta y **no** afecta si se puede facturar (verificado: `is_terminal` sigue en true). Ojo con el caso de hoy: marcar OT-5010 como "Retirado" deja la ocupación en 5, no en 4, porque su vehículo tiene además OT-977906 activa — el lugar sigue ocupado porque el vehículo sigue ahí. Lo mismo con OT-5002, cuyo vehículo tiene 3 ingresos abiertos. Es el comportamiento correcto: el lugar se libera cuando se cierra el último registro abierto de ese vehículo (verificar entrando a Facturación y comprobando que aparece en las OT facturables). Después devolverla a su estado original.
